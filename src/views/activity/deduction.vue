    ...
    <div class="m-content">
      <el-table :data="product_data" v-loading="loading" class="m-table"  stripe @selection-change="handleSelectionChange"  style="width: 100%">
        ...
        <el-table-column align="center"   label="折扣" >
          <template slot-scope="scope">
            减{{scope.row.card_price}}
          </template>
        </el-table-column>
        <el-table-column align="center" prop="medical_price"  label="使用条件" >
          <template slot-scope="scope">
            满{{scope.row.card_can_use}}
          </template>
        </el-table-column>
        <el-table-column align="center" prop="medical_for" label="已领数量/发放数量">
          <template slot-scope="scope">
            {{scope.row.card_user_get_number}}/{{scope.row.card_number}}
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" width="250" label="操作" >
          <template slot-scope="scope">
            <div class="m-flex-center">
              <div class="m-icon-div"  @click="makeCard(scope.row.card_id)">
                <!--<svg-icon icon-class="icon-editor"></svg-icon>-->
                <p>生成优惠码</p>
              </div>
              <div class="m-icon-div" @click="lookProduct(scope.row.card_id,'line')">
      ...
      <div class="m-bottom m-flex-between">
        <div class="m-btn-box">
          <span class="m-btn" @click="offline">中止</span>
    ...
    methods:{
      ...
      offline(){
        ...
        for(let i =0;i<this.multipleSelection.length;i++){
          arr.push(this.multipleSelection[i].card_id)
        }
        this.$confirm('确定要中止当前选择的活动吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          axios.post(api.offline_card,{
            card_list:arr
          }).then(res => {
            if(res.data.status == 200){
              this.getProdcut();
      ...
      lookProduct(item,name) {
        this.$router.push({path: '/activity/addDeduction',query:{card_id:item}})
      },
      //删除
      deleteProduct(){
        let arr = [];
      ...
      //生成优惠码
      makeCard(item){
        axios.get(api.make_cardno,{
          params:{
            card_id:item
          }
        }).then(res => {
          if (res.data.status == 200) {
            this.$confirm( res.data.data.cardno_no, '优惠码', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
      ...
      //  去往新增
      changeRoute(){
        this.$router.push('/activity/addDeduction')
      }
    }
  }
</script>
