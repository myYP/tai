<template>
    <div class="m-addActivity">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px" :disabled="disabled" label-position="left">
        <el-form-item label="标题" prop="card_name">
          <el-input v-model="form.card_name"></el-input>
          <p class="m-alert-text">建议问题文字在x字以内</p>
        </el-form-item>
        <el-form-item label="折扣" prop="card_price">
          <span class="m-label">减</span><el-input v-model="form.card_price" type="number" class="m-input-s"></el-input>
        </el-form-item>
        <el-form-item label="使用条件" prop="card_can_use">
          <span class="m-label">满</span><el-input v-model="form.card_can_use" type="number" class="m-input-s"></el-input>
        </el-form-item>
        <el-form-item label="发放数量" prop="card_number">
          <el-input v-model="form.card_number" type="number" class="m-input-s"></el-input>
        </el-form-item>
        <el-form-item label="有效期限" required>
          <el-col :span="6">
            <el-form-item  prop="card_start_time">
              <el-date-picker type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" v-model="form.card_start_time" style="width: 100%;"></el-date-picker>
            </el-form-item>
          </el-col>
          <el-col class="line" :span="2">-</el-col>
          <el-col :span="6">
            <el-form-item  prop="card_end_time">
              <el-date-picker type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" v-model="form.card_end_time" style="width: 100%;"></el-date-picker>
            </el-form-item>
  ...
  import api from '../../api/api'
    export default {
      ...
      methods:{
        ...
        getDetail(){
          this.loading = true;
          axios.get(api.get_card_message,{
            params: {
        ...
        saveActivity(){
          ...
          this.$refs['form'].validate((valid) => {
            if (valid) {
                axios.post(api.new_card,_form).then(res => {
                  if(res.data.status == 200){
                    this.$notify.success(res.data.message);
                    this.$router.push('/activity/deduction');
                  }else{
                    this.$message({
                      type: 'error',
